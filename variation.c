...
int* non_dominated_sorting(float ** solutions, int number_of_individuals)
{
	...
	while (allocated_solutions < number_of_individuals)
	{
		...
		for (iterator_solution=0; iterator_solution < number_of_individuals; iterator_solution++)
		{
			...
			{
				for (iterator_comparison=0; iterator_comparison < number_of_individuals; iterator_comparison++)
				{
					/* TODO PARENTESIS! if the solution is not itself, it is not been evaluated or is in the actual Pareto front */
					if (((iterator_solution != iterator_comparison) && (pareto_fronts[iterator_comparison]==0)) || (pareto_fronts[iterator_comparison]==actual_pareto_front))
					{
						/* verificate the dominance between both*/
						dominance = is_dominated(solutions,iterator_solution,iterator_comparison);
...
{
    //solutions[n][0] = Power consumption minimization
    //solutions[n][1] = Network traffic minimization
    //solutions[n][2] = Economical revenue maximization****
    //solutions[n][3] = Migration minimization

	/* if a dominates b */
	/* a better in all objectives */
	if((solutions[a][0] < solutions[b][0] && solutions[a][1] < solutions[b][1] && solutions[a][2] > solutions[b][2] && solutions[a][3] < solutions[b][3] && solutions[a][4] < solutions[b][4]) ||
	/* a better or equal in one objective and better in the others */
	solutions[a][0] <= solutions[b][0] && solutions[a][1] < solutions[b][1] && solutions[a][2] > solutions[b][2] && solutions[a][3] < solutions[b][3] && solutions[a][4] < solutions[b][4] ||
	solutions[a][0] < solutions[b][0] && solutions[a][1] <= solutions[b][1] && solutions[a][2] > solutions[b][2] && solutions[a][3] < solutions[b][3] && solutions[a][4] < solutions[b][4] ||
	solutions[a][0] < solutions[b][0] && solutions[a][1] < solutions[b][1] && solutions[a][2] >= solutions[b][2] && solutions[a][3] < solutions[b][3] && solutions[a][4] < solutions[b][4] ||
	solutions[a][0] < solutions[b][0] && solutions[a][1] < solutions[b][1] && solutions[a][2] > solutions[b][2] && solutions[a][3] <= solutions[b][3] && solutions[a][4] < solutions[b][4] ||
    solutions[a][0] < solutions[b][0] && solutions[a][1] < solutions[b][1] && solutions[a][2] > solutions[b][2] && solutions[a][3]  < solutions[b][3] && solutions[a][4] <= solutions[b][4] ||
	...
	/* if b dominates a */
	/* b better in all objectives */
	if(solutions[b][0] < solutions[a][0] && solutions[b][1] < solutions[a][1] && solutions[b][2] > solutions[a][2] && solutions[b][3] < solutions[a][3] && solutions[b][4] < solutions[a][4] ||
	/* b better or equal in one objective and better in the others */
	solutions[b][0] <= solutions[a][0] && solutions[b][1] < solutions[a][1] && solutions[b][2] > solutions[a][2] && solutions[b][3] < solutions[a][3] && solutions[b][4] < solutions[a][4] ||
	solutions[b][0] < solutions[a][0] && solutions[b][1] <= solutions[a][1] && solutions[b][2] > solutions[a][2] && solutions[b][3] < solutions[a][3] && solutions[b][4] < solutions[a][4] ||
	solutions[b][0] < solutions[a][0] && solutions[b][1] < solutions[a][1] && solutions[b][2] >= solutions[a][2] && solutions[b][3] < solutions[a][3] && solutions[b][4] < solutions[a][4] ||
	solutions[b][0] < solutions[a][0] && solutions[b][1] < solutions[a][1] && solutions[b][2] > solutions[a][2] && solutions[b][3] <= solutions[a][3] && solutions[b][4] < solutions[a][4] ||
    solutions[b][0] < solutions[a][0] && solutions[b][1] < solutions[a][1] && solutions[b][2] > solutions[a][2] && solutions[b][3]  < solutions[a][3] && solutions[b][4] <= solutions[a][4] ||
	/* b better or equal in two objectives and better in the others */
	solutions[b][0] <= solutions[a][0] && solutions[b][1] <= solutions[a][1] && solutions[b][2] > solutions[a][2] && solutions[b][3] < solutions[a][3] && solutions[b][4] < solutions[a][4] ||
	solutions[b][0] <= solutions[a][0] && solutions[b][1] < solutions[a][1] && solutions[b][2] >= solutions[a][2] && solutions[b][3] < solutions[a][3] && solutions[b][4] < solutions[a][4] ||
	solutions[b][0] <= solutions[a][0] && solutions[b][1] < solutions[a][1] && solutions[b][2] > solutions[a][2] && solutions[b][3] <= solutions[a][3] && solutions[b][4] < solutions[a][4] ||
    solutions[b][0] <= solutions[a][0] && solutions[b][1] < solutions[a][1] && solutions[b][2] > solutions[a][2] && solutions[b][3] < solutions[a][3] && solutions[b][4] <= solutions[a][4] ||
	...
	/* b better or equal in three objectives and better in the others */
	solutions[b][0] <= solutions[a][0] && solutions[b][1] <= solutions[a][1] && solutions[b][2] >= solutions[a][2] && solutions[b][3] < solutions[a][3] && solutions[b][4] < solutions[a][4] ||
    solutions[b][0] <= solutions[a][0] && solutions[b][1] <= solutions[a][1] && solutions[b][2] > solutions[a][2] && solutions[b][3] <= solutions[a][3] && solutions[b][4] < solutions[a][4] ||
	solutions[b][0] <= solutions[a][0] && solutions[b][1] <= solutions[a][1] && solutions[b][2] > solutions[a][2] && solutions[b][3] < solutions[a][3] && solutions[b][4] <= solutions[a][4] ||
    ...
    solutions[b][0] <= solutions[a][0] && solutions[b][1] <= solutions[a][1] && solutions[b][2] > solutions[a][2] && solutions[b][3] <= solutions[a][3] && solutions[b][4] <= solutions[a][4] ||
    solutions[b][0] <= solutions[a][0] && solutions[b][1] < solutions[a][1] && solutions[b][2] >= solutions[a][2] && solutions[b][3] <= solutions[a][3] && solutions[b][4] <= solutions[a][4] ||
    solutions[b][0] < solutions[a][0] && solutions[b][1] <= solutions[a][1] && solutions[b][2] >= solutions[a][2] && solutions[b][3] <= solutions[a][3] && solutions[b][4] <= solutions[b][4])
	{
		return -1;
	}
...
int** mutation(int **population, int **V, int number_of_individuals, int h_size, int v_size)
{
	...
	for (iterator_individual = 0 ; iterator_individual < number_of_individuals ; iterator_individual++)
	{
		...
		for (iterator_virtual = 0 ; iterator_virtual < v_size ; iterator_virtual++)
		{
			...
			//probability = drand48() *1.0; Windows
			...
			if (probability < (float)1/v_size)
			{
			    ...
			    if (V[iterator_virtual][0] > 0){
                    ...
                    if (V[iterator_virtual][3] == 2)
                    {
                        physical_position = generate_solution_position(h_size, 1);
                    }
                    else
                    {
                        physical_position = generate_solution_position(h_size, 0);
                    }
...
int** population_evolution(int **PQ_aux, float **objectives_functions_PQ_aux, int **P, int **Q, float **objectives_functions_P, float **objectives_functions_Q, int *fronts_P, int number_of_individuals, int v_size)
{
	...
	float **objectives_functions_PQ = objectives_functions_PQ_aux;
	...
	for (iterator_individual_P=0; iterator_individual_P < number_of_individuals; iterator_individual_P++)
	{
		...
		//objectives_functions_PQ[iterator_individual_P] = (float *) malloc (5 *sizeof (float));
		...
		objectives_functions_PQ[iterator_individual_P][3] = objectives_functions_P[iterator_individual_P][3];
		objectives_functions_PQ[iterator_individual_P][4] = objectives_functions_P[iterator_individual_P][4];
	}
	...
	for (iterator_individual_P=number_of_individuals; iterator_individual_P < 2 * number_of_individuals; iterator_individual_P++)
	{
		...
		objectives_functions_PQ[iterator_individual_P][3] = objectives_functions_Q[iterator_individual_Q][3];
		objectives_functions_PQ[iterator_individual_P][4] = objectives_functions_Q[iterator_individual_Q][4];
