 ...
 * returns: T matrix
 */
int** load_T(int v_size, char path_to_file[], int t_set_number)
{
	/* datacenter file to read from */
	FILE *datacenter_file;
	/* line readed from file */
	char input_line[1000000];
	/* iterators */
	int iterator_row = 0;
	int iterator_column = 0;

	/* the current T set we are reading */
	int current_t_set = 0;
	...
	if (datacenter_file != NULL)
	{
		...
		{
			/* get line per line */
			fgets(input_line, 1000000, datacenter_file);
			...
			if (current_t_set > t_set_number)
            {
                break;
            }
			/* if we are in the correct set and it is not a blank line, we count */
			if ((current_t_set == t_set_number) && strcmp(input_line, "\n") != 0)
			{
